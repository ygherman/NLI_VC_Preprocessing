...
def create_ROOT_id(df):
    ...
    if "ROOTID" in list(df.columns):
        ...
        if len(df[df["LEVEL"] == "Fonds Record"]) == 1:
            df.loc[df["LEVEL"] == "Fonds Record", "ROOTID"] = ""

        elif len(df[df["LEVEL"] == "Fonds Record"]) > 1:
            logger.error(
...
def add_normal_dates_to_section_record(df, collection_id):
    ...
    if pd.isnull(df.loc[collection_id, "DATE_START"]) and pd.isnull(
        ...
        elif date == "יושלם בהמשך":
            df["DATE_START"] = ""
            df["DATE_END"] = ""
            df["NOTES_HIDDEN"] = "תאריכים יושלמו בהמשך;" + df["NOTES_HIDDEN"].astype(
                str
            )
    return df
...
def main():
    ...
    if "LANGUAGE" in list(collection.full_catalog.columns):
        logger.info(f"[LANGUAGE] Starting to work on LANGUAGE column")
        collection.full_catalog = check_values_against_cvoc(
            collection.full_catalog,
            "LANGUAGE",
            Authority_instance.language_mapping_dict,
        )
        logger.info(f"[LANGUAGE] Creating language Match File")
        create_match_file(
            collection,
            Authority_instance.df_languages,
            authority_Excelfile(collection.full_catalog, "LANGUAGE"),
            "LANGUAGE",
        )
    else:
        logger.error(f"[LANGUAGE] no columns names [LANGUAGE] in table!")
    ...
    logger.info(f"Execution Time: {convert(elapsed)}")
